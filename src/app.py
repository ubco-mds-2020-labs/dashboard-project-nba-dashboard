...
player_data = pd.read_csv('https://github.com/ubco-mds-2020-labs/nba_analytics/raw/main/data/players_stats_by_season_full_details.csv', usecols = lambda x: x not in columns_to_skip)
...
NBA_data = pd.read_csv("https://github.com/ubco-mds-2020-labs/nba_analytics/raw/main/data/players_stats_by_season_full_details.csv")
...
def simple_stat(stat):
    ...
    axis=alt.Axis(grid=False), title=None), alt.X('Season')).configure_view(strokeWidth=0).properties(width=430, height = 160)
...
def type_stat(stat):
    ...
    header= alt.Header(labelOrient='bottom', labelAngle=90, labelPadding=60)), alt.Color('Type', legend = alt.Legend(orient = 'bottom', title=None))).configure_facet(spacing=5).configure_axis(grid=False).configure_view(strokeWidth=0).properties(width=42, height = 145)
